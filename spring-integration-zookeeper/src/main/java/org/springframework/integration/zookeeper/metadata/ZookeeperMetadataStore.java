				...
				switch (event.getType()) {
					case CHILD_ADDED:
						...
						for (MetadataStoreListener listener : ZookeeperMetadataStore.this.listeners) {
							listener.onAdd(getKey(event.getData().getPath()), Conversions.bytesToString(event.getData().getData(), encoding));
						}
						break;
					case CHILD_UPDATED:
						if (ZookeeperMetadataStore.this.updateMap.containsKey(getKey(event.getData().getPath()))) {
						...
						for (MetadataStoreListener listener : ZookeeperMetadataStore.this.listeners) {
							listener.onUpdate(getKey(event.getData().getPath()), Conversions.bytesToString(event.getData().getData(), encoding));
						}
						break;
					case CHILD_REMOVED:
						ZookeeperMetadataStore.this.updateMap.remove(getKey(event.getData().getPath()));
						for (MetadataStoreListener listener : ZookeeperMetadataStore.this.listeners) {
							listener.onRemove(getKey(event.getData().getPath()), Conversions.bytesToString(event.getData().getData(), encoding));
						}
						break;
					default:
