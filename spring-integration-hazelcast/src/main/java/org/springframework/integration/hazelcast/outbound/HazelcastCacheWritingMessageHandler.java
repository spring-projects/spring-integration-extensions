...
public class HazelcastCacheWritingMessageHandler extends AbstractMessageHandler implements BeanPostProcessor, DisposableBean {
	...
	private void writeToCache(Message<?> message) {
		if(distributedObject instanceof IMap) {
			((IMap<?,?>)distributedObject).putAll((Map) message.getPayload());
		} else if(distributedObject instanceof IList) {
			((IList<?>)distributedObject).addAll((List) message.getPayload());
		} else if(distributedObject instanceof ISet) {
			((ISet<?>)distributedObject).addAll((Set) message.getPayload());
		} else if(distributedObject instanceof IQueue) {
			((IQueue<?>)distributedObject).addAll((Queue) message.getPayload());
		} 
	}

	@Override
	public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
		if (!HazelcastIntegrationDefinitionValidator.validateCacheType(distributedObject)) {
