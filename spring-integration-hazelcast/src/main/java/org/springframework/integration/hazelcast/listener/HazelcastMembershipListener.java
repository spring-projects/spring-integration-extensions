...
public class HazelcastMembershipListener implements MembershipListener {
	...
	public void memberRemoved(MembershipEvent membershipEvent) {
		...
		if(!hazelcastLocalInstanceSet.isEmpty()) {
			...
			Lock lock = hazelcastInstance.getLock(ApplicationContextStartEventHandler.HZ_INTERNAL_CONFIGURATION_MULTI_MAP_LOCK);
			lock.lock();
			try {
				...
				if(configMultiMap.keySet().contains(removedMemberSocketAddress)) {
					SocketAddress newAdminSocketAddress = getNewAdminInstanceSocketAddress(configMultiMap, removedMemberSocketAddress);
