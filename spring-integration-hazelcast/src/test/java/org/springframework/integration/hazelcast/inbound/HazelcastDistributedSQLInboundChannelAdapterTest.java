...
@ContextConfiguration(locations={"/HazelcastDistributedSQLInboundChannelAdapterTest-context.xml"})
@DirtiesContext(classMode = ClassMode.AFTER_CLASS)
public class HazelcastDistributedSQLInboundChannelAdapterTest {
	...
	public void testDistributedSQLForOnlyENTRYIterationType() throws InterruptedException {
		...
		Assert.assertEquals(4, (((Map.Entry)((Collection)msg.getPayload()).iterator().next()).getKey()));
		Assert.assertEquals(4, ((User)((Map.Entry)((Collection)msg.getPayload()).iterator().next()).getValue()).getId());
		Assert.assertEquals("TestName4", ((User)((Map.Entry)((Collection)msg.getPayload()).iterator().next()).getValue()).getName());
		Assert.assertEquals("TestSurname4", ((User)((Map.Entry)((Collection)msg.getPayload()).iterator().next()).getValue()).getSurname());
	}
	...
	@Test
	public void testDistributedSQLForOnlyLOCAL_KEYIterationType() throws InterruptedException {
	...
	public void testDistributedSQLForOnlyVALUEIterationType() throws InterruptedException {
		...
		Assert.assertNotNull(msg.getPayload());
		Assert.assertTrue(msg.getPayload() instanceof Collection);
		Assert.assertEquals(3, ((User)(((Collection)msg.getPayload()).iterator().next())).getId());
		Assert.assertEquals("TestName3", ((User)(((Collection)msg.getPayload()).iterator().next())).getName());
		Assert.assertEquals("TestSurname3", ((User)(((Collection)msg.getPayload()).iterator().next())).getSurname());
	}
}
